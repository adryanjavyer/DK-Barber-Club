const BARBERSHOP = {
  whatsappNumber: "5538998825000", 
  city: "Belo Horizonte/MG",
};

const BOOKINGS_STORAGE_KEY = "dkBarberBookings";
const BOOKINGS_REMOTE_TABLE = window.DK_BOOKINGS_TABLE || "dk_bookings";
...
function canUseRemoteBookings() {
  return Boolean(BOOKINGS_REMOTE_TABLE && getAdminSupabaseClient());
}

function getRemoteBookingRow(booking) {
  const bookingData = {
    ...booking,
  ...
  return {
    ...
    booking_date_iso: bookingData.bookingDateIso || null,
    booking_time: bookingData.time || null,
    booking_time_start: getTimeStart(bookingData.time || "") || null,
    client_name: bookingData.clientName || null,
    client_phone: bookingData.clientPhone || null,
    service_name: bookingData.serviceName || null,
    professional_name: bookingData.professionalName || null,
    status: bookingData.status || "Pendente",
    source: bookingData.source || "Site",
    updated_at: new Date().toISOString(),
  };
}

function getBookingFromRemoteRow(row) {
  if (!row) return null;

  if (row.booking_data && typeof row.booking_data === "object") {
    return {
      ...row.booking_data,
      id: row.id || row.booking_data.id,
      status: row.status || row.booking_data.status,
      updatedAt: row.updated_at || row.booking_data.updatedAt,
    };
  }
...
async function insertBookingInRemoteDatabase(booking) {
  if (!booking?.id || !canUseRemoteBookings()) return false;

  try {
    const supabaseClient = getAdminSupabaseClient();
    const { error } = await supabaseClient
      .from(BOOKINGS_REMOTE_TABLE)
      .insert(getRemoteBookingRow(booking));

    if (error) {
      if (error.code !== "23505") {
        console.warn("Não foi possível salvar o agendamento no Supabase. Usando cópia local.", error);
      }
      return false;
    }
...
async function saveBookingToRemoteDatabase(booking) {
  if (!booking?.id || !canUseRemoteBookings()) return false;
  ...
  try {
    ...
    const { error } = await supabaseClient
      .from(BOOKINGS_REMOTE_TABLE)
      .upsert(getRemoteBookingRow(booking), { onConflict: "id" });

    if (error) {
      console.warn("Não foi possível atualizar o agendamento no Supabase. Mantendo cópia local.", error);
      return false;
    }
...
async function deleteBookingFromRemoteDatabase(id) {
  if (!id || !canUseRemoteBookings()) return false;
  ...
  try {
    ...
    if (error) {
      console.warn("Não foi possível excluir o agendamento no Supabase. Removendo apenas da cópia local.", error);
      return false;
    }
...
async function fetchRemoteBookingsFromDatabase() {
  ...
  try {
    ...
    if (error) {
      console.warn("Não foi possível carregar agendamentos do Supabase. Exibindo cópia local.", error);
      return null;
    }

    return (data || []).map(getBookingFromRemoteRow).filter(Boolean);
  } catch (error) {
    console.warn("Não foi possível carregar agendamentos do Supabase. Exibindo cópia local.", error);
    return null;
  }
...
const professionals = [
  ...
  {
    id: "Beatriz",
    name: "Beatriz Cassemiro",
    role: "Especialista em cortes femininos",
    initial: "B",
    note: "Atendimento rápido",
  },
];
...
function saveCurrentBooking() {
  ...
  if (!alreadyExists) {
    ...
    insertBookingInRemoteDatabase(record).then((savedRemotely) => {
      if (savedRemotely && typeof window.refreshEmbeddedAdminPanel === "function") {
...
function updateCurrentBookingStatus(status) {
  ...
  const bookings = getStoredBookings().map((booking) => {
    if (booking.id !== state.bookingId) return booking;

    updatedBooking = { ...booking, status, updatedAt: new Date().toISOString() };
  ...
  if (updatedBooking) {
    saveBookingToRemoteDatabase(updatedBooking);
  }
}
...
let lastRenderedStep = state.currentStep;
...
function buildBookingWhatsAppUrl() {
  ...
  const isTodayBooking = day?.iso === toISODate(getTodayStart());
  const priorityLine = isTodayBooking ? "⚠️ Atendimento para hoje (prioridade)." : "";
...
function buildCancelWhatsAppUrl() {
  ...
  const isTodayBooking = day?.iso === toISODate(getTodayStart());
  const priorityLine = isTodayBooking ? "⚠️ Agendamento de hoje (prioridade)." : "";
...
Se possível, me confirmem o cancelamento.
Obrigado!`
  );
...
function updateUI() {
  const stepDirection = state.currentStep >= lastRenderedStep ? "forward" : "back";
  ...
  screens.forEach((screen) => {
    ...
    if (isActive && state.currentStep !== lastRenderedStep) {
      void screen.offsetWidth;
      screen.classList.add(stepDirection === "forward" ? "screen-enter-forward" : "screen-enter-back");
    }
  });
...
function initEmbeddedAdminPanel() {
  ...
  const dailyCalendarView = $("#dailyCalendarView");
  const dailyAgendaMonth = $("#dailyAgendaMonth");
  const dailyAgendaDayNumber = $("#dailyAgendaDayNumber");
  const dailyAgendaWeekday = $("#dailyAgendaWeekday");
  const dailyAgendaWeekLabel = $("#dailyAgendaWeekLabel");
  const bookingDayFilter = $("#bookingDayFilter");
  const dailyPrevDayButton = $("#dailyPrevDayButton");
  const dailyNextDayButton = $("#dailyNextDayButton");
  const dailyTodayButton = $("#dailyTodayButton");
  ...
  const manualQuickOpenButton = $("#manualQuickOpenButton");
  const manualBookingDetails = $("#manualBookingDetails");
  ...
  async function syncAdminBookingsFromRemote({ renderAfterSync = true } = {}) {
    if (!(await isAdminAuthenticated())) return false;

    const remoteBookings = await fetchRemoteBookingsFromDatabase();
    if (!Array.isArray(remoteBookings)) return false;

    const remoteNormalized = remoteBookings.map(normalizeAdminBooking);
    const localNormalized = getStoredBookings().map(normalizeAdminBooking);
    const mergedMap = new Map();

    remoteNormalized.forEach((booking) => mergedMap.set(booking.id, booking));
    localNormalized.forEach((booking) => {
      if (!mergedMap.has(booking.id)) mergedMap.set(booking.id, booking);
    });

    adminState.bookings = Array.from(mergedMap.values()).sort((a, b) => getBookingDateTime(a) - getBookingDateTime(b));
  ...
  function getFilteredBookings() {
    ...
    const selectedDay = adminState.filters.day || adminTodayIso();
    ...
    return adminState.bookings.filter((booking) => {
      ...
      const matchesDay = booking.bookingDateIso === selectedDay;

      return matchesSearch && matchesStatus && matchesDay;
    }).sort((a, b) => getBookingDateTime(a) - getBookingDateTime(b));
  }

  function getAllBookingsForSelectedDay() {
    const selectedDay = adminState.filters.day || adminTodayIso();

    return adminState.bookings
      .filter((booking) => booking.bookingDateIso === selectedDay)
      .sort((a, b) => getBookingDateTime(a) - getBookingDateTime(b));
  }

  function adminFormatCalendarMonth(isoDate) {
    return new Intl.DateTimeFormat("pt-BR", { month: "long" }).format(adminParseIsoDate(isoDate));
  }

  function adminFormatWeekdayLong(isoDate) {
    return new Intl.DateTimeFormat("pt-BR", { weekday: "long" }).format(adminParseIsoDate(isoDate));
  }
  ...
  function setSelectedBookingDay(isoDate) {
    adminState.filters.day = isoDate || adminTodayIso();

    if (bookingDayFilter) {
      bookingDayFilter.value = adminState.filters.day;
    }

    renderBookings();
  }

  function shiftSelectedBookingDay(days) {
    const date = adminParseIsoDate(adminState.filters.day || adminTodayIso());
    date.setDate(date.getDate() + days);
    setSelectedBookingDay(toISODate(date));
  }

  function getEventToneClass(booking) {
    const serviceId = String(booking.serviceId || "").replace(/[^a-z0-9-]/gi, "");
    if (serviceId) return `event-service-${serviceId}`;
    return "event-service-default";
  }
  ...
  function renderBookings() {
    ...
    const allDayBookings = getAllBookingsForSelectedDay();
    const selectedDay = adminState.filters.day || adminTodayIso();
    const selectedDate = adminParseIsoDate(selectedDay);
    ...
    const timelineHeight = (calendarEndHour - calendarStartHour + 1) * hourHeight;

    if (bookingDayFilter && bookingDayFilter.value !== selectedDay) {
      bookingDayFilter.value = selectedDay;
    }

    if (dailyAgendaMonth) {
      dailyAgendaMonth.textContent = adminFormatCalendarMonth(selectedDay);
    }

    if (dailyAgendaDayNumber) {
      dailyAgendaDayNumber.textContent = String(selectedDate.getDate()).padStart(2, "0");
    }

    if (dailyAgendaWeekday) {
      dailyAgendaWeekday.textContent = adminFormatWeekdayLong(selectedDay);
    }

    if (dailyAgendaWeekLabel) {
      dailyAgendaWeekLabel.textContent = `SEM. ${getISOWeekNumber(selectedDate)}`;
    }

    emptyBookingsState.style.display = bookings.length ? "none" : "block";

    if (!dailyCalendarView) return;
    ...
    const eventCards = bookings.map((booking) => {
      const start = getTimeStart(booking.time) || "00:00";
      const end = getTimeEnd(booking.time) || "";
      const startMinutes = timeToMinutes(start);
      const endMinutes = end ? timeToMinutes(end) : startMinutes + 30;
      const safeStart = Math.max(dayStartMinutes, Math.min(startMinutes, dayEndMinutes - 15));
      const safeEnd = Math.max(safeStart + 30, Math.min(endMinutes, dayEndMinutes));
      const groupKey = String(safeStart);
      const overlapIndex = eventsByStart.get(groupKey) || 0;
      eventsByStart.set(groupKey, overlapIndex + 1);

      const top = Math.max(0, ((safeStart - dayStartMinutes) / 60) * hourHeight) + (overlapIndex * 6);
      const height = Math.max(42, ((safeEnd - safeStart) / 60) * hourHeight - 6);
      const horizontalOffset = overlapIndex * 10;
      const toneClass = getEventToneClass(booking);

      return `
        <details
          class="daily-event-card ${toneClass} ${statusClass(booking.status)}"
          style="--event-top:${top}px; --event-height:${height}px; --event-left:${horizontalOffset}px;"
        >
          <summary>
            <span class="daily-event-client">${booking.clientName}</span>
            <span class="daily-event-meta">${start}${end ? ` - ${end}` : ""} · ${booking.serviceName}</span>
          </summary>

          <div class="daily-event-expanded">
            <span><strong>Profissional:</strong> ${booking.professionalName}</span>
            <span><strong>Valor:</strong> ${adminFormatCurrency(booking.servicePriceValue)}</span>
            <span><strong>Origem:</strong> ${booking.source}</span>
    ...
    dailyCalendarView.innerHTML = `
      <div class="daily-calendar-stage" style="--daily-hour-height:${hourHeight}px; --daily-stage-height:${timelineHeight}px;">
        <div class="daily-hours-layer" aria-hidden="true">
          ${hours.map((hour) => `
            <div class="daily-hour-line">
              <span>${hour}</span>
            </div>
          `).join("")}
        </div>

        <div class="daily-events-layer">
          ${eventCards}
        </div>
      </div>

      ${isDateClosed(selectedDate) && !allDayBookings.length ? `
        <div class="daily-closed-note">
          <strong>Barbearia fechada neste dia.</strong>
          <span>Use as setas ou escolha outra data.</span>
        </div>
      ` : ""}
    `;
  }
  ...
  function updateBookingStatus(id, status) {
    ...
    saveAdminBookings();
    if (updatedBooking) saveBookingToRemoteDatabase(updatedBooking);
    renderAdmin();
    renderTimes();
  }
  ...
  function deleteBooking(id) {
    ...
    saveAdminBookings();
    deleteBookingFromRemoteDatabase(id);
    renderAdmin();
    renderTimes();
  }
  ...
  function populateManualForm() {
    ...
    if (bookingDayFilter) bookingDayFilter.value = adminState.filters.day || adminTodayIso();
  ...
  function addManualBooking(event) {
    ...
    adminState.filters.day = bookingDateIso;
    saveAdminBookings();
    saveBookingToRemoteDatabase(booking);
    manualBookingForm.reset();
    populateManualForm();
    if (bookingDayFilter) bookingDayFilter.value = bookingDateIso;
    if (manualBookingDetails) manualBookingDetails.open = false;
    renderAdmin();
    renderTimes();
    setActiveAdminTab("bookings");

    setTimeout(() => {
      dailyCalendarView?.scrollIntoView({ behavior: "smooth", block: "start" });
    }, 0);
  }
  ...
  function bindAdminEvents() {
    ...
    bookingDayFilter?.addEventListener("change", (event) => {
      setSelectedBookingDay(event.target.value || adminTodayIso());
    });

    dailyPrevDayButton?.addEventListener("click", () => shiftSelectedBookingDay(-1));
    dailyNextDayButton?.addEventListener("click", () => shiftSelectedBookingDay(1));
    dailyTodayButton?.addEventListener("click", () => setSelectedBookingDay(adminTodayIso()));
    manualQuickOpenButton?.addEventListener("click", () => {
      if (!manualBookingDetails) return;
      manualBookingDetails.open = true;
      manualBookingDetails.scrollIntoView({ behavior: "smooth", block: "center" });
      setTimeout(() => manualClientName?.focus(), 180);
    });
    ...
    dailyCalendarView?.addEventListener("change", handleAdminActions);
    dailyCalendarView?.addEventListener("click", handleAdminActions);
...
/* =========================================================
   APPLE-INSPIRED MOTION LAYER
   Progressive enhancement only: all booking/admin behavior
   above remains the source of truth.
   ========================================================= */
(function initAppleInspiredMotion() {
  ...
  // Let the first paint settle before revealing content, avoiding a flash.
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      root.dataset.appleMotion = "ready";
    });
  });

  // Subtle optical response to pointer position. Intentionally capped so
  // it supports depth without turning the UI into a cursor-following effect.
  if (hero && window.matchMedia?.("(pointer: fine)").matches) {
    hero.addEventListener("pointermove", (event) => {
      ...
      root.style.setProperty("--hero-shift-x", `${x.toFixed(2)}px`);
      root.style.setProperty("--hero-shift-y", `${y.toFixed(2)}px`);
    }, { passive: true });
  ...
  // Small scroll-linked depth cue with a stable frame of reference.
  let scrollRaf = 0;
  const syncScrollMotion = () => {
    scrollRaf = 0;
    if (!motionAllowed()) {
      root.style.setProperty("--hero-scroll", "0px");
      return;
    }
    const y = Math.min(window.scrollY || 0, 220);
    root.style.setProperty("--hero-scroll", `${(y * 0.035).toFixed(2)}px`);
  };

  window.addEventListener("scroll", () => {
    if (!scrollRaf) scrollRaf = requestAnimationFrame(syncScrollMotion);
  }, { passive: true });

  // Native-feeling press feedback for interactive surfaces.
  const pressSelector = [
    ".option-card",
    ...
    ".whatsapp-button",
    ".cancel-button",
    ".admin-primary-button:not(:disabled)",
    ".admin-tab",
    ".more-menu-button",
    ".calendar-nav-button:not(:disabled)",
    ".daily-icon-button",
    ".period-summary-card"
  ].join(",");
  ...
  // When reduced-motion changes while the page is open, immediately reset
  // transform-driven effects instead of requiring a reload.
  ...
  // Safari/iOS-style keyboard comfort: keep focused fields above floating UI.
  document.addEventListener("focusin", (event) => {
    if (!event.target.matches?.("input, select, textarea")) return;
    if (window.innerWidth > 720) return;
    setTimeout(() => {
      event.target.scrollIntoView({ block: "center", behavior: motionAllowed() ? "smooth" : "auto" });
    }, 140);
  });

  // Add semantic state that can also be used by future CSS enhancements.
  body?.classList.add("apple-motion-enhanced");
})();
