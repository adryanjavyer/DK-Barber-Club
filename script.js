...
// Chave usada como cópia local de segurança.
// O salvamento principal dos agendamentos usa o Supabase quando a tabela dk_bookings estiver criada.
...
async function insertBookingInRemoteDatabase(booking) {
  ...
  try {
    ...
    if (error) {
      // Código 23505 = já existe. Neste caso, não interrompe o fluxo do cliente.
      if (error.code !== "23505") {
        console.warn("Não foi possível salvar o agendamento.", error);
      }
      return false;
    }
...
async function saveBookingToRemoteDatabase(booking) {
  ...
  try {
    ...
    if (error) {
      console.warn("Não foi possível atualizar o agendamento.", error);
      return false;
    }
...
async function deleteBookingFromRemoteDatabase(id) {
  ...
  try {
    ...
    if (error) {
      console.warn("Não foi possível excluir o agendamento.", error);
      return false;
    }
...
async function fetchRemoteBookingsFromDatabase() {
  ...
  try {
    ...
    if (error) {
      console.warn("Não foi possível carregar agendamentos.", error);
      return null;
    }
...
const BUSINESS_HOURS_BY_WEEKDAY = {
  0: [],
  1: [{ start: "07:30", end: "11:00" }, { start: "13:00", end: "19:00" }],
  2: [{ start: "07:30", end: "11:00" }, { start: "13:00", end: "19:00" }],
  3: [{ start: "07:30", end: "11:00" }, { start: "13:00", end: "19:00" }],
  4: [{ start: "07:30", end: "11:00" }, { start: "13:00", end: "19:00" }],
  5: [{ start: "07:30", end: "11:00" }, { start: "13:00", end: "19:00" }],
...
function getTimeUnavailableReason(timeRange) {
  ...
  if (isTimeAlreadyBooked(timeRange)) return "Agendado";
  return "";
}
