...
function initEmbeddedAdminPanel() {
  ...
  const dailyAgendaSummary = $("#dailyAgendaSummary");
  const dailyAgendaTitle = $("#dailyAgendaTitle");
  const dailyAgendaSubtitle = $("#dailyAgendaSubtitle");
  ...
  function adminFormatLongDate(isoDate) {
    if (!isoDate) return "";
  ...
  function renderBookings() {
    ...
    const daySlots = getAvailableTimesForDate(selectedDay);
    const timesSet = new Set([
      ...daySlots,
      ...allDayBookings.map((booking) => booking.time).filter(Boolean),
    ]);
    const timelineTimes = Array.from(timesSet).sort((a, b) => timeToMinutes(getTimeStart(a)) - timeToMinutes(getTimeStart(b)));
    const bookingsByTime = new Map();

    bookings.forEach((booking) => {
      const time = booking.time || "Horário não informado";
      const current = bookingsByTime.get(time) || [];
      current.push(booking);
      bookingsByTime.set(time, current);
    });
    ...
    if (dailyAgendaTitle) {
      dailyAgendaTitle.textContent = adminFormatLongDate(selectedDay);
    }

    if (dailyAgendaSubtitle) {
      const total = allDayBookings.length;
      const visible = bookings.length;
      dailyAgendaSubtitle.textContent = total === visible
        ? `${total} agendamento${total === 1 ? "" : "s"} no dia`
        : `${visible} de ${total} agendamento${total === 1 ? "" : "s"} exibido${visible === 1 ? "" : "s"}`;
    }

    const activeBookings = bookings.filter((booking) => booking.status !== "Cancelado");
    const doneBookings = bookings.filter((booking) => booking.status === "Concluído");
    const canceledBookings = bookings.filter((booking) => booking.status === "Cancelado");

    if (dailyAgendaSummary) {
      dailyAgendaSummary.innerHTML = `
        <article>
          <span>Exibidos</span>
        ...
        <article>
          ...
          <small>Pendentes, confirmados ou concluídos</small>
        </article>
        <article>
          <span>Concluídos</span>
          <strong>${doneBookings.length}</strong>
          <small>${adminFormatCurrency(doneBookings.reduce((sum, booking) => sum + (Number(booking.servicePriceValue) || 0), 0))}</small>
        </article>
        <article>
          <span>Cancelados</span>
          <strong>${canceledBookings.length}</strong>
          <small>Fora do faturamento</small>
        </article>
      `;
    }
    ...
    if (isDateClosed(selectedDate) && !allDayBookings.length) {
      dailyCalendarView.innerHTML = `
        <div class="daily-closed-state">
          <strong>Barbearia fechada neste dia.</strong>
          <p>Escolha outro dia no campo acima ou use as setas para navegar pela agenda.</p>
        </div>
      `;
      return;
    }

    const rows = timelineTimes.length ? timelineTimes : daySlots;

    dailyCalendarView.innerHTML = rows.length
      ? rows.map((time) => {
        const slotBookings = bookingsByTime.get(time) || [];
        const slotStart = getTimeStart(time);
        const slotEnd = getTimeEnd(time);
        const isUnavailableSlot = !isTimeInsideBusinessHoursForDate(time, selectedDate);

        return `
          <div class="daily-time-row ${slotBookings.length ? "has-booking" : ""} ${isUnavailableSlot ? "outside-hours" : ""}">
            <div class="daily-time-cell">
              <strong>${slotStart || time}</strong>
              ${slotEnd ? `<span>${slotEnd}</span>` : ""}
            </div>
            <div class="daily-slot-cell">
              ${slotBookings.length ? slotBookings.map((booking) => `
                <article class="daily-booking-card ${statusClass(booking.status)}">
                  <div class="daily-booking-top">
                    <div>
                      <span class="daily-booking-label">Cliente</span>
                      <strong>${booking.clientName}</strong>
                    </div>
                    ${statusPill(booking.status)}
                  </div>

                  <div class="daily-booking-details">
                    <span><strong>Horário:</strong> ${booking.time}</span>
                    <span><strong>Serviço:</strong> ${booking.serviceName}</span>
                  ...
                  <div class="daily-booking-controls">
                    ${createStatusSelect(booking)}
                    ${createActions(booking)}
                  </div>
                </article>
              `).join("") : `
                <div class="daily-empty-slot">
                  <span>Horário livre</span>
                  <small>${isUnavailableSlot ? "Fora do horário padrão" : "Disponível para agendamento"}</small>
                </div>
              `}
            </div>
          </div>
      ...
      : `
        <div class="daily-closed-state">
          <strong>Nenhum horário disponível para este dia.</strong>
          <p>Verifique as regras de funcionamento ou escolha outra data.</p>
        </div>
      `;
  }
